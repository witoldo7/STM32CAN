...
bool (*hscan_rx_cb)(void *, void *, packet_t *);
bool (*swcan_rx_cb)(void *, void *, packet_t *);

j2534_conn *swconn = NULL;
j2534_conn *hsconn = NULL;
...
bool canBaudRate(CANConfig *can_cfg, uint32_t can_baudrate, uint32_t *sjw, uint32_t *bsp)
{
   ...
   *
   */
  if ((can_baudrate != (pclk / (prescaler * (1 + bs1 + bs2)))) || (bs1 < 1) || (bs1 > MaxBS1) || (bs2 < 1) || (bs2 > MaxBS2))
  {
    return false;
  }

  // Configure the interface
  can_cfg->NBTP = (0 << FDCAN_NBTP_NSJW_Pos) | ((bs1 - 1) << FDCAN_NBTP_NTSEG1_Pos) | ((bs2 - 1) << FDCAN_NBTP_NTSEG2_Pos) | ((prescaler - 1) << FDCAN_NBTP_NBRP_Pos);
