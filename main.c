...
static THD_FUNCTION(usb_rx, arg) {
	...
	while (TRUE) {
		...
		if (*(receiveBuf) != 0) {
			...
			uint16_t len = (uint16_t) ((receiveBuf[1] & 0xffffU) << 8)
					| (uint16_t) receiveBuf[2];
...
static THD_FUNCTION(combi, arg) {
	...
	while (TRUE) {
		...
		if (rx_packet.term == cmd_term_ack) {
			...
			if (ret != true) {
				CombiSendReplyPacket(&tx_packet, &rx_packet, 0, 0,
						cmd_term_nack);
			}
...
static THD_FUNCTION(can_rx, p) {
	...
	packet_t tx_packet = { .data = packetbuff, .cmd_code = cmd_can_frame,
			.data_len = 15 };
...
int main(void) {
	...
	chThdCreateStatic(usb_rx_wa, sizeof(usb_rx_wa), NORMALPRIO + 7, usb_rx,
			NULL);
	chThdCreateStatic(combi_wa, sizeof(combi_wa), NORMALPRIO + 7, combi, NULL);
	chThdCreateStatic(can_rx_wa, sizeof(can_rx_wa), NORMALPRIO + 20, can_rx,
			NULL);
