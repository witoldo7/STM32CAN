...
void sleep_us(int usec) {
	...
	timer = NULL; //CreateWaitableTimer(NULL, TRUE, NULL);
...
semaphore_t semaphore_create(char* semname) {
	(void)semname;
	return NULL; //CreateSemaphore(NULL, 0, 1, semname);
}
...
void convertPacketToPMSG(uint8_t *data, uint16_t len, PASSTHRU_MSG* pMsg) {
	...
	case ISO9141:
	    uint8_t data_len = 0;
	    memcpy(&data_len, data+2, sizeof(data_len));
		pMsg->ProtocolID = protocol;
		pMsg->DataSize = data_len;
		pMsg->ExtraDataIndex = 0;
		memcpy(pMsg->Data, data + 3, data_len+1);
	break;
	default:
